package f_exchange;
import java.sql.*;
public class SellExchangeTransaction extends Transaction implements TransactionCreateable {
    ...
    public String createTransaction(int usr_id, int currency_id, double amount) {
        ...
        String usr_thb_acc_credit_res;
        String admin_thb_acc_dedit_res;
        String usr_fxc_acc_debit_res;
        String admin_fxc_acc_credit_res;
                ...
                if(currency_exist.equals("Currency found successfully!!")) {
                   ...
                   if(user_thb_acc_exist.equals("THB Account found successfully!!") && admin_thb_acc_exist.equals("THB Account found successfully!!")
                      && user_fxc_acc_exist.equals("FXC Account found successfully!!") && admin_fxc_acc_exist.equals("FXC Account found successfully!!")){
                       ...
                       usr_thb_acc_credit_res = user_thb_acc.credit(user_thb_acc.getId(), this.getCurrency().getSell_rate() * amount, usr_id);
                       admin_thb_acc_dedit_res = admin_thb_acc.debit(admin_thb_acc.getId(), this.getCurrency().getSell_rate() * amount, admin.getId());
                      
                       usr_fxc_acc_debit_res = user_fxc_acc.debit(user_fxc_acc.getId(), amount, usr_id);
                       admin_fxc_acc_credit_res = admin_fxc_acc.credit(admin_fxc_acc.getId(), amount, admin.getId());
                       
                       if(usr_thb_acc_credit_res.equals("Credited successfully!!") && admin_thb_acc_dedit_res.equals("Debited successfully!!")
                          && usr_fxc_acc_debit_res.equals("Debited successfully!!") && admin_fxc_acc_credit_res.equals("Credited successfully!!")){
                          insertTrn(usr_id, user_fxc_acc.getId(), currency_id, user.getUsername(), amount, this.getCurrency().getSell_rate(), user_thb_acc.getId());
    ...
    public void insertTrn(int usr_id, int user_fxc_acc_id, int currency_id, String user_username, double amount, double rate,int user_thb_acc_id) throws SQLException {
        Connection con = DBConnection.connectDB();
        PreparedStatement insert_query = con.prepareStatement("insert into transaction (user_id, user_fxc_acc_id, currency_id, transaction_desc, transaction_type, amount, rate, user_thb_acc_id) values (?, ?, ?, ?, 'Exchange Sell', ?, ?, ?)");
        insert_query.setInt(1, usr_id);
        insert_query.setInt(2, user_fxc_acc_id);
        insert_query.setInt(3, currency_id);
        insert_query.setString(4, user_username + " Sells to F_Exchange");
